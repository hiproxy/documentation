...
var localizedPath = ['configuration', 'get_started', 'developers', 'api'];
...
//     console.log('path::', path, title, localizedPath);
...
hexo.extend.helper.register('header_menu', function (className) {
  var menu = this.site.data.nav;
  var result = '';
  var self = this;
  var lang = this.page.lang;
  var isEnglish = lang === 'en';
  var currMenu = menu[lang];

  _.each(currMenu, function(path, title) {
    if (path && path.url) {
      path = path.url;
    }

    if (!isEnglish) path = lang + path; 
